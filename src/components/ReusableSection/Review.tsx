...
export default function Review() {
  ...
  useEffect(() => {
    ...
    if (currentIndex > max) setCurrentIndex(max < 0 ? 0 : max);
  }, [itemsPerView]);
  ...
  // Shift = currentIndex * (cardWidth + gap)
  const shiftCalc = `calc(${currentIndex} * ((100% - ${(itemsPerView - 1) * GAP}px) / ${itemsPerView} + ${GAP}px))`;
            ...
            <button
              ...
              className={`hidden md:flex flex-shrink-0 w-10 h-10 items-center justify-center rounded-full transition-all duration-200 z-10 ${
                currentIndex === 0
                  ...
                  : "cursor-pointer bg-orange-500 shadow-lg hover:bg-orange-600 hover:scale-110"
              }`}
            >
